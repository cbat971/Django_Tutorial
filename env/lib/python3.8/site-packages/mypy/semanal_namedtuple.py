...
class NamedTupleAnalyzer:
    ...
    def parse_namedtuple_args(
        ...
        if not isinstance(args[0], (StrExpr, BytesExpr, UnicodeExpr)):
            self.fail(
                '"{}()" expects a string literal as the first argument'.format(
                    type_name
                ),
                call,
            )
        ...
        if underscore:
            self.fail(
                '"{}()" field names cannot start with an underscore: '.format(type_name)
                + ", ".join(underscore),
                call,
            )
        if len(defaults) > len(items):
            self.fail(
                'Too many defaults given in call to "{}()"'.format(type_name), call
            )
